...
**Models (OpenRouter):**
- **Main SproutAI interface** (general chat + product-analysis suggestions): `google/gemini-2.5-pro` – strong reasoning, large context.
...
### 2.3.1 Company facts grounding (report enrichment)
...
*Implementation: `frontend/src/services/openRouter.js` (API: `DEFAULT_MODEL` = Gemini 2.5 Pro for main SproutAI and product analysis, `POPUP_MODEL` = Perplexity Sonar Pro for popup chat and optional company-facts grounding, `REPORT_MODEL` = Claude 3.5 Sonnet for report summary + report writing); `SproutAIPage.jsx` (chat + report); `AIChatPopup` (Dashboard/Analytics popup).*
